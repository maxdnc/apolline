...
const HamburgerIcon = ({ isOpen }: HamburgerIconProps) => {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center">
      ...
      />
      <span
        className={`block w-6 h-0.5 rounded bg-gray-800 transition-all duration-100 ease-[cubic-bezier(0.83, 0, 0.17, 1)]
        ${isOpen ? 'opacity-0 scale-50' : 'opacity-100 delay-100'}`}
      />
      <span
        className={`block w-6 h-0.5 rounded bg-gray-800 transition-all
